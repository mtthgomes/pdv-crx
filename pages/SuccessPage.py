...
class SuccessPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        success_img = tk.PhotoImage(file="images/success.png")
        ...
        self.create_custom_button(button_frame, "Reimprimir Comprovante", self.reimprimir_comprovante)

        self.create_custom_button_error(button_frame, "Finalizar Compra", self.controller.reiniciar_aplicacao)

    def set_numero_comprovante(self, numero_comprovante):
        """Função para configurar o número do comprovante"""
        self.numero_comprovante = numero_comprovante

    def reimprimir_comprovante(self):
        """Função para reimprimir o comprovante"""
        if self.numero_comprovante:
            comprovante_path = f"C:\\ClientLinxTEF\\Cupons\\{self.numero_comprovante}.001"
            Imprimir_comprovante(comprovante_path)
        else:
            print("Número do comprovante não definido.")

    def create_custom_button_error(self, parent, text, command):
